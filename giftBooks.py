...
def get_results(the_lcc):
    ...
    if the_lcc:
        if the_lcc in cinematography:
            #print(the_lcc, "jenny")
            window['-OUTPUT-'].update(the_lcc, "jenny")
        else:
            ...
            #print(f"{the_lcc}\n{the_code}")
            window['-OUTPUT-'].update(the_lcc, the_code)

            if selector_dict.get(the_code):
                #print(selector_dict[the_code])
                window['-OUTPUT-'].update(selector_dict[the_code])
            else:
                #print("Unknown Collector")
                window['-OUTPUT-'].update("Unknown Selector")

    else:
        #print("No lcc found.")
        window['-OUTPUT-'].update("No lcc found")
...
# while True:
#
#     u_input = input("Enter ISBN or title words [or q to quit]: ")
#
#     print(u_input)
#
#
...
#
#         if new_isbn := il.isbn_from_words(u_input):
#             print("Found isbn: ", new_isbn)
#         if lcc := il.classify(new_isbn).get("lcc"):
#             print("Found lcc: ", lcc)
#         get_results(lcc)
#
#
...
layout = [[sg.Text('Scan ISBN (or type title words):'), sg.Text(size=(15,1), key='-OUTPUT-')],
          [sg.Input(key='-IN-')],
          [sg.Button('Show'), sg.Button("Clear"), sg.Button('Exit')]]


# layout = [  [sg.Text('Scan barcode OR ')],
#             [sg.Text('Enter title words: '), sg.InputText()],
#             [sg.Button('Ok'), sg.Button('Cancel'), sg.Button("Clear")] ]

# Create the Window
window = sg.Window('Selector Parser', layout)
