...
window.title("Welcome to Selector-Selector!")
#window.resizable(width=False, height=False)

frm_entry = tk.Frame(master=window)
u_input = tk.Entry(master=frm_entry, width=10)

lbl_temp = tk.Label(master=frm_entry, text="\N{DEGREE FAHRENHEIT}")
u_input.grid(row=0, column=0, sticky="e")

frm_entry.grid(row=0, column=0, padx=10)
lbl_temp.grid(row=0, column=1, sticky="w")

isbn_or_title = u_input.get()


btn_lookup = tk.Button(
    master=window,
    text="\N{RIGHTWARDS BLACK ARROW}",
    command=get_results(isbn_or_title)
...
frm_entry.grid(row=0, column=0, padx=10)
btn_lookup.grid(row=0, column=1, pady=10)
