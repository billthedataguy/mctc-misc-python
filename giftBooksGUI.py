...
layout = [[sg.Text(''), sg.Text(size=(25,1), key='-OUTPUT-')],
          [sg.Input(key='-IN-')],
          [sg.Button('Find'), sg.Button('Clear'), sg.Button('Exit')]]
...
while True:  # Event Loop
    ...
    if event == 'Find' or values["-IN-"]:
        u_input = values["-IN-"].strip()

        if is_title(u_input):
            the_isbn = get_isbn(u_input)
            the_lcc = get_lcc(the_isbn)
            the_selector = get_results(the_lcc)
            the_message = f"{the_lcc} {the_selector}"
