    ...
    if the_lcc:
        ...
        if in_cinematography(the_lcc):
            ...
            if selector_dict.get(the_code):
                g.msgbox(the_lcc + " ---> " + selector_dict[the_code])
            else:
                g.msgbox("Unknown Selector")

    else:
        g.msgbox("No lcc found. Try title search instead.")


# TR848-TR861 jenny CINEMATOGRAPHY

msg = "Enter ISBN or title/author/edition words: "
title = "Selector Sorter"

while True:

    u_input = g.enterbox(msg, title, strip=True)
    ...
    if u_input is None:
        ...
        get_results(get_lcc(u_input))

    elif is_title(u_input):
        get_results(get_lcc(get_isbn(u_input)))
