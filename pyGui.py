...
layout = [[sg.Text('Info:'), sg.Text(size=(25,1), key='-OUTPUT-')],
          [sg.Input(key='-IN-')],
          [sg.Button('Find'), sg.Button('Exit')]]

window = sg.Window('SelectorSelector', layout)
...
while True:  # Event Loop
    ...
    if event == sg.WIN_CLOSED or event == 'Exit':
        break
    if event == 'Find':
        u_input = values["-IN-"].strip()

        if is_title(u_input):
            new_isbn = il.isbn_from_words(u_input) if u_input else None
                #window['-OUTPUT-'].update(new_isbn)
                #print("Found isbn: ", new_isbn)
            lcc = il.classify(new_isbn).get("lcc")
                    #window['-OUTPUT-'].update(lcc)
                    #print("Found lcc: ", lcc)
            the_message = new_isbn + " " + lcc + " " + get_results(lcc)
            window['-OUTPUT-'].update(the_message)

        elif is_isbn(u_input):
            #window['-OUTPUT-'].update(u_input)
            lcc = il.classify(u_input).get("lcc")
            the_message = u_input + " " + lcc + " " + get_results(lcc)
            window['-OUTPUT-'].update(the_message)
            #print(get_results(lcc))

        else:
            window['-OUTPUT-'].update("Something's wrong, VERY wrong")
