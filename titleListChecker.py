...
root = tk.Tk()
root.title("Simple Library Catalog Title List Checker")
...
def search_primo():
    ...
    if titles_str:
        ...
        for title in title_list:
            ...
            try:
                num_recs = r.json()["info"]["total"]
                if int(num_recs) > 0:
                    title_textbox.insert(tk.END, f'<b>{r.json()["info"]["total"]} rec(s) found.</b>\n\n<p></p>')
                else:
...
def save_html():
    filename = f"title_checker_results_{date.today()}.html"
    with open(filename, "w") as f:
        f.write(f'<b>{date.today()} Title Check</b><br><br>')
